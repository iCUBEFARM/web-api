...
# USERS_FILE = os.path.join(os.path.dirname(FILE_DIR), "data", "users-with-resumes-latest-csv.csv")   # latest user file with resume info
# USERS_FILE = os.path.join(os.path.dirname(FILE_DIR), "data", "users-without-countrycodes-resumes-latest-6.csv")  # latest user file with resume info
# USERS_FILE = os.path.join(os.path.dirname(FILE_DIR), "data", "user-without-country codes-jobseekers-ex.csv")  # latest user file with resume info
# USERS_FILE = os.path.join(os.path.dirname(FILE_DIR), "data", "lastest-users-from-production.csv")   # latest extra users file with resume info
# USERS_FILE = os.path.join(os.path.dirname(FILE_DIR), "data", "latest_extra_failed_users.csv")   # latest 18 extra failed  users file with resume info
# USERS_FILE = os.path.join(os.path.dirname(FILE_DIR), "data", "lastest-users-nov26.csv")   # Nov 26th 43 users
USERS_FILE = os.path.join(os.path.dirname(FILE_DIR), "data", "latest-users-dec7th-production-csv.csv")   # Dec 7th 83 users


WRITE_USER_SAVED_FILE = os.path.join(os.path.dirname(FILE_DIR), "logs", "users-save-success.txt")
WRITE_USER_UNSAVED_FILE = os.path.join(os.path.dirname(FILE_DIR), "logs", "users-save-failed.txt")
...
#     # csv.DictReader uses first line in file for column headings by default
    ...
    try:
        ...
        for row in dr:
            if row['status'] == '2':
                uid = row['uid'].lstrip().rstrip()
                email = row['email'].lstrip().rstrip()
                # password = row['password'].lstrip().rstrip()
                password = 'pbkdf2_sha256$36000$GUTXz8HgMnAP$gHieLsSShEm2LOLHl043O96SPhBAmT8Bphh0mDlcdAw='
                first_name = row['first_name'].lstrip().rstrip()
                last_name = row['last_name'].lstrip().rstrip()
                registration_date = row['registration_date'].lstrip().rstrip()
                status = row['status'].lstrip().rstrip()
                primary_telephone_number = row['primary_telephone_number'].lstrip().rstrip()
                telephone_code = row['telephone_code'].lstrip().rstrip()
                # last_login_date = row['last_login_date']
                last_login_date = '9/29/18 00:00'
                preferred_language = row['prefered_lang'].lstrip().rstrip()
                profile_image_url = row['profile_image_url'].lstrip().rstrip()

                telephone_code = telephone_code
                primary_telephone_number = primary_telephone_number
                # primary_telephone_number = re.sub(r"[-()\"#/@;:%<>*${}$&`=~^|.!?, ]", "", primary_telephone_number)
                if telephone_code:
                    mobile = "+{code}{number}".format(code=telephone_code, number=primary_telephone_number)
                    mobile = re.sub(r"[-()\"#/@;:%<>*${}$&`=~^|.!?, ]", "", mobile)
                else:
                    # If number does not start with + AND length of the number is greater than 11
                    if len(primary_telephone_number) >= 11:
                        if not primary_telephone_number.startswith('+'):
                            mobile = "+{number}".format(number=primary_telephone_number)
                            mobile = re.sub(r"[-()\"#/@;:%<>*${}$&`=~^|.!?, ]", "", mobile)
                    else:
                        logger.info("cannot process the phone no  - {email}\n".format(email=email))
                ...
                try:
                ...
                except Language.DoesNotExist as ldn:
                    user_save_failure_file.write("{email},{exception}\n".format(email=email, exception=ldn))
                    pass
                    ...
                    logger.info("Could not create user with - {email},{exception}\n".format(email=email, exception=e))
                    pass
